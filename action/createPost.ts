...
export async function createPost({
  ...
  try {
    ...
    const isMember = await checkUserMembership(subreddit._id, user._id);
    if (!isMember) {
      console.log(`User ${user._id} is not a member of subreddit ${subreddit._id}`);
      return { error: "You must join this community before you can create a post." };
    }
