...
export async function POST(req: NextRequest, context: { params: { slug: string } } | { params: Promise<{ slug: string }> }) {
  ...
  const { userToUnmod } = await req.json();
  ...
  const memberIdx = community.members?.findIndex((m: any) => m.user && m.user._ref === userToUnmod && m.role === "moderator");
  if (memberIdx === -1 || memberIdx === undefined)
    return NextResponse.json({ error: "Not a moderator" }, { status: 400 });

  await adminClient
    .patch(community._id)
    .set({ [`members[${memberIdx}].role`]: "member" })
    .commit();
